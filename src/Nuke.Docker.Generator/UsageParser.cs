...
namespace Nuke.Docker.Generator
{
    public static class UsageParser
    {
        ...
        {
            return Parse(usage.Split(separator: ' ', options: StringSplitOptions.RemoveEmptyEntries));
        }
        ...
        public static List<UsageParameter> Parse(string[] parameters)
        {
            ...
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                if (match.Success)
                {
                    ...
                    if (!value.StartsWith(value: '[') && parameters.Length > i + 1
                                                      && parameters[i + 1]
                                                      == $"[{match.Groups[groupnum: 1].Value}={match.Groups[groupnum: 2].Value}...]") i++;
                }

                else if (value.StartsWith(value: '[') && value.EndsWith("...]"))
                    usage.IsList = true;
